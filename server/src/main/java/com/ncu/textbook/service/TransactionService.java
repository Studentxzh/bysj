...
public class TransactionService {
    ...
    public List<SaleOrderView> listAllSaleOrdersForAdmin() {
        return transactionRecordMapper.findAllSaleOrders().stream()
                .map(this::toAdminListView)
                .collect(Collectors.toList());
    }
    ...
    private SaleOrderView toAdminListView(TransactionRecord rec) {
        Textbook tb = textbookMapper.findById(rec.getTextbookId());
        SaleOrderView v;
        if (tb == null) {
            v = new SaleOrderView();
            ...
            v.setTextbookTitle("（教材已删除）");
            v.setSellerId(rec.getSellerId());
            Student seller = studentMapper.findById(rec.getSellerId());
            v.setSellerRealName(seller != null ? seller.getRealName() : null);
        } else {
            v = toView(rec, tb);
        }
        v.setBuyerId(rec.getBuyerId());
        Student buyer = studentMapper.findById(rec.getBuyerId());
        v.setBuyerRealName(buyer != null ? buyer.getRealName() : null);
        return v;
    }
}
